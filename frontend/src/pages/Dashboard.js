...
import {
  ...
  Settings, GraduationCap, Star, FileText, Home
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [activePkg, setActivePkg]       = useState(() => {
    try { return localStorage.getItem('activePackage') || 'YKS'; }
    catch { return 'YKS'; }
  });
  ...
  useEffect(() => {
    ...
    if (!isTeacher) {
      ...
      api.get(`/api/spaced-repetition/today/${userId}`)
        .then(res => {
          const pkgSubjects = PKG_SUBJECTS[activePkg] || PKG_SUBJECTS.YKS;
          const filtered = (res.data || []).filter(r =>
            !r.topic?.course?.name || pkgSubjects.includes(r.topic.course.name)
          );
      ...
      <aside className="w-64 shrink-0 bg-[#1a2540] flex flex-col shadow-2xl z-20">
        ...
        <div className="px-6 py-5 border-b border-white/10">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate('/')}>
            <div className="w-9 h-9 bg-gradient-to-br from-orange-400 to-orange-600 rounded-xl flex items-center justify-center shadow group-hover:scale-105 transition-transform">
              <GraduationCap size={18} className="text-white" />
            </div>
            <div>
              <p className="text-white font-bold text-sm leading-none group-hover:text-orange-300 transition-colors">SmartExam</p>
              ...
              <div className="flex gap-3">
                {PACKAGES.map(p => {
                  ...
                  if (owned) {
                    ...
                    return (
                      <button key={p.id}
                        onClick={() => { setActivePkg(p.id); localStorage.setItem('activePackage', p.id); }}
                  ...
                  return (
                    <div key={p.id}
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">{p.sub}</p>
                        <button
                          onClick={() => { purchasePkg(p.id); setActivePkg(p.id); localStorage.setItem('activePackage', p.id); }}
