...
export default function LandingPage() {
  ...
  const ownedPackages  = JSON.parse(localStorage.getItem('ownedPackages') || '["YKS"]');
            ...
            {isLoggedIn ? (
              <>
                ...
                <button onClick={() => navigate('/dashboard')}
                  className="bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition shadow-lg shadow-orange-500/30 flex items-center gap-1.5">
        ...
        {mobileMenu && (
          <div className="md:hidden bg-[#1a2540] border-t border-white/10 px-6 py-4 space-y-3">
            ...
            <div className="flex flex-col gap-2 pt-2 border-t border-white/10">
              {isLoggedIn ? (
                <button onClick={() => navigate('/dashboard')}
                  className="bg-orange-500 text-white text-sm font-semibold py-2.5 rounded-xl">
                  Dashboard'a Git
                </button>
              ) : (
                <>
      ...
      <section id="paketlerimiz" className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {pkg.plans.map((plan) => {
              const pkgOwned = isLoggedIn && ownedPackages.includes(activePkg) && !plan.comingSoon;
              return (
              <div key={plan.name}
                className={`relative bg-white rounded-2xl border shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl overflow-hidden
                  ...
                  ${pkgOwned ? 'ring-2 ring-emerald-400/40' : ''}`}>
                {plan.highlight && (
                ...
                <div className="p-6">
                  ...
                  {pkgOwned ? (
                    <>
                      <button onClick={() => navigate('/courses', { state: { pkg: activePkg } })}
                        className="w-full py-3 rounded-xl font-semibold text-sm transition bg-emerald-500 hover:bg-emerald-600 text-white shadow-md shadow-emerald-200 flex items-center justify-center gap-2">
                        <CheckCircle size={15} /> Çalışmaya Başla →
                      </button>
                      <p className="text-center text-xs text-emerald-600 font-semibold mt-2">
                        ✓ Satın Alındı — Kalan Süre: 365 Gün
                      </p>
                    </>
                  ) : (
                    <button onClick={() => navigate(plan.comingSoon ? '/' : isLoggedIn ? '/courses' : '/register', { state: { pkg: activePkg } })}
                      ...
                      {plan.comingSoon ? 'Yakında' : isLoggedIn ? 'Pakete Git →' : 'Ücretsiz Başla →'}
                    </button>
                  )}
                </div>
              </div>
